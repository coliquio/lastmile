...
openssl req -x509 -new -nodes -key $FILE_PATH.ca.key -days 9999 -out $FILE_PATH.ca.crt -subj "/C=DE/ST=Konstanz/L=Konstanz/O=Global Security/OU=IT Department/CN=example.com"
...
openssl x509 -req -days 9999 -in $FILE_PATH.srv.csr -CA $FILE_PATH.ca.crt -CAkey $FILE_PATH.ca.key -set_serial 01 -out $FILE_PATH.srv.crt
...
openssl x509 -req -days 9999 -in $FILE_PATH.srv-selfsigned.csr -signkey $FILE_PATH.srv-selfsigned.key -out $FILE_PATH.srv-selfsigned.crt
